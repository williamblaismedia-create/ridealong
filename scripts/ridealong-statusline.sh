...
alive=0
if [ -f "$HOME/.claude/ridealong-live.json" ]; then
  pid="$(python3 -c 'import json,os; print(json.load(open(os.path.expanduser("~/.claude/ridealong-live.json"))).get("pid",0))' 2>/dev/null || echo 0)"
