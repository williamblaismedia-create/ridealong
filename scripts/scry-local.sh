#!/usr/bin/env bash
#
# Scry — tout-en-un LOCAL : Chrome + serveur MCP sur la machine que l'on
# pilote (pas de serveur distant, pas de tunnel). Lancé PAR Claude Code :
#   claude mcp add scry -- /chemin/vers/scry/scripts/scry-local.sh
#
...
SCRY_DATA_DIR="${SCRY_DATA_DIR:-$HOME/scry-donnees}"
export SCRY_DATA_DIR
mkdir -p "$SCRY_DATA_DIR"
ENV_FILE="$SCRY_DATA_DIR/scry.env"
if [ ! -f "$ENV_FILE" ]; then
  secret="$(od -An -N32 -tx1 /dev/urandom | tr -d ' \n')"
  printf 'SCRY_CDP_URL=http://127.0.0.1:9222\nSCRY_LIVE_PORT=9400\nSCRY_LIVE_SECRET=%s\n' "$secret" > "$ENV_FILE"
  chmod 600 "$ENV_FILE"
  echo "scry-local: config creee dans $ENV_FILE" >&2
fi
...
if ! curl -s -m 2 "http://127.0.0.1:${CDP_PORT}/json/version" >/dev/null 2>&1; then
  ...
  [ -n "$CHROME" ] || { echo "scry-local: Chrome introuvable (definir SCRY_CHROME)" >&2; exit 127; }
  echo "scry-local: demarrage de Chrome ($CHROME) sur le port CDP $CDP_PORT" >&2
...
[ -x "$NODE_BIN" ] || { echo "scry-local: node introuvable" >&2; exit 127; }
[ -f "$HERE/dist/src/server.js" ] || { echo "scry-local: dist/ absent — lancer 'npm run build' dans $HERE" >&2; exit 1; }
